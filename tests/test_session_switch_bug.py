...
def test_session_not_killed_on_model_switch():
    async def _run():
        ...
        try:
            ...
            # Simulate model switch (frontend calls updateConfig)
            ...
            # Let both sessions run a bit
            await asyncio.sleep(0.2)

            # Check session A's turns
            resp = await client.get(f"/sessions/{sid_a}/turns")
            assert resp.status == 200
            turns_a = (await resp.json())["turns"]
            print(f"Session A turns: {turns_a}")
