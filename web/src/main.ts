...
function editComposerPending(sid: string, itemId?: string) {
  ...
  // (A when editing B) must stay queued. The pre-queue tail used to call
  // setSessionPending(sid, null) here, which deleted the whole session
  // queue, and re-read queuedImages() (by then the HEAD = another item's
  // images) into the draft. Both relics are gone; do not reintroduce them.
  if (ta) ta.focus();
}
