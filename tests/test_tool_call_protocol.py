...
def test_structured_tool_call_protocol():
    async def _run():
        ...
        rt = Runtime.create(workspace_root=root, model_adapter=StructuredToolAdapter())
...
def test_invalid_tool_yields_recoverable_flow():
    async def _run():
        root = Path(__file__).resolve().parents[1]
        rt = Runtime.create(workspace_root=root, model_adapter=InvalidToolAdapter())
...
def test_max_rounds_guardrail():
    async def _run():
        ...
        rt = Runtime.create(
            workspace_root=root,
            model_adapter=InfiniteToolAdapter(),
